...
CREATE TABLE IF NOT EXISTS ` + "`content_generations`" + ` (
    ...
    ` + "`user_id`" + ` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '归属用户 ID (关联 users.id)',
    ` + "`kind`" + ` VARCHAR(32) NOT NULL COMMENT 'video / image / audio',
    ` + "`model`" + ` VARCHAR(128) NOT NULL,
    ` + "`prompt`" + ` TEXT NULL COMMENT '提示词纯文本快照',
    ...
    INDEX ` + "`idx_cg_user_created`" + ` (` + "`user_id`" + `, ` + "`created_at`" + ` DESC),
    INDEX ` + "`idx_cg_billing_scan`" + ` (` + "`billing_status`" + `, ` + "`status`" + `),
